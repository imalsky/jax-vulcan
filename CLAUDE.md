...
- **Step-count and atom-conservation drift downstream of the `chem_rhs` floor.** The per-term ulp floor isn't visible in a single-step test, but it shows up in two places after a long integration:
    1. *JAX takes more accepted steps to declare convergence than master.* The convergence test fires when the worst species's `longdy = max |Δy/(n_0·ymix)|` drops below `yconv_min` (loose branch) or `yconv_cri` (strict branch). The ~1e-4 floor doesn't move bulk species, but it nudges a handful of cancellation-prone heavy hydrocarbons (`C6H6`, `C2H2`, `C4H5`, `C4H2`, `C3H3`, `CH3NH2`, ...) along slightly different trajectories. Whichever one of those happens to be sitting at the threshold last is the one that gates termination. Master's gating species crosses the threshold at e.g. step ~1000 with `longdy ≈ 0.05`; JAX's gating species crosses at e.g. step ~1500–3000 with `longdy ≈ 0.09`. Both runs reach physically equivalent steady states; only the detection moment differs.
    2. *JAX's column atom_loss grows larger than master's because of (1).* Each accepted step adds a small (~1e-4 relative) ulp drift to a few species; with `loss_eps = 0.1` master never rejects the step. The drift is approximately linear in step count, so a 2.8× longer run ends with 2.8× more accumulated drift on the affected atoms. On HD189 both runs sit at ~2e-4 column drift; on HD209 with `conver_ignore=[]` JAX accumulates to −8.3% C while master sits at +5e-5. This is *not* a per-step accuracy regression — it's the ulp floor compounded over more steps.
  Both effects share two mitigations, which are now wired in:
    * `conver_ignore` (master and JAX, populated in the upstream-default `cfg_examples/vulcan_cfg_HD189.py` and `vulcan_cfg_HD209.py`) lists the heavy-hydrocarbon offenders so they don't gate the convergence detector. The default list is `['C6H6', 'C2H2', 'C6H5', 'C2H', 'C2H4', 'C2H5', 'C2H6', 'C3H2', 'C3H3', 'C4H5', 'CH2NH', 'CH3NH2', 'H2CCO']`. Extend per-config if a *new* trace radical takes over the gating role on a different planet/network.
    * `conv_stall_window = 200` (default) is a stall fallback in `op.stop` (master) and `outer_loop.cond_fn` (JAX). It tracks the running min of `longdy` (only resets on a ≥5% relative drop) and fires `end_case=1` when `count_since_new_min > conv_stall_window` *and* both `longdy_seen_min < yconv_min` *and* current `longdy < yconv_min` *and* `aflux_change < flux_cri`. The dual `seen_min` + current-`longdy` gate stops the run from terminating while a new species is genuinely still evolving — it only fires when chemistry has been oscillating around the converged state for too long. Master almost never trips it (master's regular detector fires earlier); JAX trips it when a heavy hydrocarbon outside `conver_ignore` has been bouncing around the threshold for 200+ accepted steps.
  These two knobs are the right answer for tightening JAX↔master timing/conservation parity without rewriting `chem_rhs`. Don't reach for tighter `loss_eps` (causes dt thrashing without reducing cumulative drift — drift is per physical time, not per step) or compensated summation (CLAUDE.md's chem_rhs bullet documents that this is a no-op against SymPy's per-term values).
