...
n_indx["O2"] = lambda lmd: (
    (20564.8 + 2.480899e13 / (4.09e9 - (1.0 / (lmd / 1.0e7)) ** 2.0)) * 1.0e-8 + 1.0
)
n_ref["O2"] = 2.68678e19
King["O2"] = lambda lmd: (
    1.09
    + 1.385e-11 * (1.0 / (lmd / 1.0e7)) ** 2.0
    + 1.448e-20 * (1.0 / (lmd / 1.0e7)) ** 4.0
)

n_indx["N2"] = lambda lmd: (
    (5677.465 + 3.1881874e14 / (1.44e10 - (1.0 / (lmd / 1.0e7)) ** 2.0)) * 1.0e-8 + 1.0
)
n_ref["N2"] = 2.546899e19
King["N2"] = lambda lmd: 1.034 + 3.17e-12 * (1.0 / (lmd / 1.0e7))

n_indx["CO2"] = lambda lmd: (
    (
        5799.25 / (128908.9**2 - (1.0 / (lmd / 1.0e7)) ** 2)
        + 120.05 / (89223.8**2 - (1.0 / (lmd / 1.0e7)) ** 2)
        + 5.3334 / (75037.5**2 - (1.0 / (lmd / 1.0e7)) ** 2)
        + 4.3244 / (67837.7**2 - (1.0 / (lmd / 1.0e7)) ** 2)
        + 0.1218145e-6 / (2418.136**2 - (1.0 / (lmd / 1.0e7)) ** 2)
    )
    * 1.1427e3
    + 1.0
)
n_ref["CO2"] = 2.546899e19
King["CO2"] = lambda lmd: 1.1364 + 25.3e-12 * (1.0 / (lmd / 1.0e7)) ** 2.0

n_indx["He"] = lambda lmd: (
    (2283.0 + (1.8102e13) / (1.5342e10 - (1.0 / (lmd / 1.0e7)) ** 2.0)) * 1.0e-8 + 1
)
