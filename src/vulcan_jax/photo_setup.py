...
def _discover_T_cross_files(sp: str) -> list[int]:
    ...
    T_list: list[int] = []
    folder = str(resolve_data_path("thermo/photo_cross/" + sp + "/"))
...
def _bin_T_dependent(
    ...
    bins: np.ndarray,
    Tco: np.ndarray,
    cross_at_bins: np.ndarray,
    cross_J_at_bins: dict[int, np.ndarray],
    ...
    cross_J_T = {
        i: np.zeros((nz, nbin), dtype=np.float64) for i in range(1, n_branch + 1)
    }
    ...
    if use_ion:
        for sp in ion_sp:
            ...
            for i in range(1, var.ion_branch[sp] + 1):
                ...
                cross_Jion_per_key[(sp, i)] = cross_Jion_at_bins * ratio_at_bins
