...
def photo_data_from_static(static, species_list) -> PhotoData:
    ...
    if absp_sp_non_T:
        absp_idx = np.array(
            [species_list.index(sp) for sp in absp_sp_non_T],
            dtype=np.int64,
        )
    ...
    if absp_T_sp_list:
        absp_T_idx = np.array(
            [species_list.index(sp) for sp in absp_T_sp_list],
            dtype=np.int64,
        )
    else:
    ...
    if scat_sp_list:
        scat_idx = np.array(
            [species_list.index(sp) for sp in scat_sp_list],
            dtype=np.int64,
        )
...
def compute_tau_jax(y: jnp.ndarray, dz: jnp.ndarray, photo: PhotoData) -> jnp.ndarray:
    ...
    if photo.absp_idx.shape[0] > 0:
        abs_per_layer = (
            jnp.einsum("js,sb->jb", y[:, photo.absp_idx], photo.absp_cross)
            * dz[:, None]
        )
    else:
    ...
    if photo.absp_T_idx.shape[0] > 0:
        absT_per_layer = (
            jnp.sum(
                y[:, photo.absp_T_idx][:, :, None]
                * photo.absp_T_cross.transpose(1, 0, 2),
                axis=1,
            )
            * dz[:, None]
        )
    ...
    if photo.scat_idx.shape[0] > 0:
        scat_per_layer = (
            jnp.einsum("js,sb->jb", y[:, photo.scat_idx], photo.scat_cross)
            * dz[:, None]
        )
...
def _compute_J_inner(aflux, cross_J, cross_J_T, din12_indx, dbin1, dbin2):
    ...
    if cross_J_T.shape[0] > 0:
        ...
        JT1 = (
            JT1
            - 0.5
            * (flux1[:, 0][None] * cT1[:, :, 0] + flux1[:, -1][None] * cT1[:, :, -1])
            * dbin1
        )
        ...
        JT2 = (
            JT2
            - 0.5
            * (flux2[:, 0][None] * cT2[:, :, 0] + flux2[:, -1][None] * cT2[:, :, -1])
            * dbin2
        )
        J_br_T = JT1 + JT2
    else:
...
def update_k_with_J(
    ...
    safe_J_br = jnp.where(branch_active[:, None], J_br_scaled, k_arr[0][None, :])
    safe_J_br_T = jnp.where(branch_T_active[:, None], J_br_T_scaled, k_arr[0][None, :])
