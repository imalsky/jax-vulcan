...
### `vulcan_jax.py`
Driver script (~80 lines). Mirrors `vulcan.py`'s orchestration. Sets the JAX
...
- `validate_runtime_config(cfg, root=None)` — top-level entry, called from `vulcan_jax.py` and `outer_loop.OuterLoop._call_legacy`.
...
run `python vulcan_jax.py`, or import via `from cfg_examples.vulcan_cfg_HD189
