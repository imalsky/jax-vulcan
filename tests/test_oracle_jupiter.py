"""Jupiter-config oracle: 20-step Ros2 comparison vs VULCAN-master.

Runs the vendored ``cfg_examples/vulcan_cfg_Jupiter.py`` end-to-end (atm +
...
Jupiter mirrors HD189 closely (``ini_mix='EQ'``, ``use_photo=True``) but
uses a different network (``NCHO_photo_network_lowT_Jupiter.txt``) and
atmosphere (``Jupiter_deep_top.txt``), with low-T rate caps active and
condensation enabled (H2O, NH3).
...
# tighter than 1e-9 on Jupiter's smooth atmospheric column would be
# surprising and worth investigating.
COUNT_MAX = 20

# Master-vs-JAX comparison: 1e-9 absorbs the chem_rhs cancellation floor.
RELERR_VS_MASTER = 1e-9
...
def test_oracle_jupiter() -> None:
    """Jupiter-config 20-step Ros2 oracle. See module docstring for skips."""
