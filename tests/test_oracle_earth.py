"""Earth-config oracle: 20-step Ros2 comparison vs VULCAN-master.

Runs the vendored ``cfg_examples/vulcan_cfg_Earth.py`` end-to-end (atm +
...
Earth is the first config that exercises ``use_condense=True`` with
``T_cross_sp=['CO2','H2O','NH3']`` and ``ini_mix='const_mix'`` —  paths
HD189 doesn't touch.
...
# tighter than 1e-9 on Earth's smooth pre-condensation phase would be
...
def test_oracle_earth() -> None:
    """Earth-config 20-step Ros2 oracle. See module docstring for skips."""
