...
### Post-review fixes (same pass, after a three-reviewer adversarial audit)

- **`use_ion` electron-row freeze ported.** Master's Ros2 solver pins the electron rows inside both stages when `use_ion=True` (`op.py:2949-2952`, `2966-2967`: `k1[e]=k2[e]=0`, so `sol[e]=y[e]` and `delta[e]=0` within the step; `e` is recomputed only by the post-step charge balance). VULCAN-JAX previously implemented only the post-step charge balance, letting `e` evolve through the linear solve and contribute to the truncation-error gate — a live-physics semantics deviation on every ion run. The runner now ORs a constant electron-column mask into the existing `fix_mask` row-pin plumbing when `use_ion` (without the `fix_y` overwrite, which is fix-species-only).
- **Legacy-entry crash with T-dependent cross sections fixed.** The legacy `(var, atm, para)` call path synthesizes a RunState with no `photo_static` slot, so the new per-profile photo arrays in `ProfileVars` fell back to `(0,1,1)` placeholders and crashed the photo branch whenever `T_cross_sp` was non-empty. The packer now falls back to the closure-baked `_PhotoStatic` arrays on that (single-profile-only) path — value-identical to the pre-ProfileVars behavior.
- **Same-star guard hole closed.** `_sflux_top_ref` was only recorded by `prepare_runstate`, so a runner pre-built by a single-profile run would accept a later different-star batch profile (silently integrating it with the baked star's flux). The reference is now recorded in `_build_photo_static` itself, covering every entry path.
