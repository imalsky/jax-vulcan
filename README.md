...
- `use_print_delta` (master's largest-truncation-error print inside the solver) is not supported: a per-step host print is impractical inside the JIT'd runner. The key is removed from the config surface and rejected with a migration message (as are `fix_species_time` — the pin gate is `stop_conden_time` — and `gs`)
