...
def _emit_rate_term(
    net: Network, slot_i: int, max_terms: int, PAD: int
) -> str:
...
def emit_chem_rhs_source(net: Network) -> str:
    ...
    lines.append("")
    lines.append("")
    lines.append("chem_rhs_codegen = jax.vmap(chem_rhs_per_layer_codegen, in_axes=(0, 0, 1))")
    lines.append("")
    return "\n".join(lines)
...
def chem_rhs_cache_key(net: Network) -> str:
    ...
    for arr in (net.reactant_idx, net.product_idx,
                net.reactant_stoich, net.product_stoich,
                net.is_three_body, net.is_forward):
...
def build_chem_rhs(net: Network) -> Callable:
    ...
    fn = jax.jit(ns["chem_rhs_codegen"])
    _BUILD_CACHE[key] = fn
    return fn
...
if __name__ == "__main__":
    ...
    net = parse_network(vulcan_cfg.network)
    path = cache_path_for(net)
    if path.exists():
